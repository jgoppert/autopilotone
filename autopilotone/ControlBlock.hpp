...
public:
	...
	public:
		PIDBlock(float kp, float ki, float kd, float cutoffFreq_radps,
				float iSat, float ySat) :
				m_ySat(ySat), m_integral(ki, iSat), m_proportional(kp), m_derivative(kd), m_filter(
						cutoffFreq_radps) {
		}
		...
		virtual float update(float input, float dt) {
			...
			output = SaturationBlock::saturate(output, m_ySat);
